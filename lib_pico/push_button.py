...
class Button():   
    def __init__(self, name, gpio, pull=-1,
                 interrupt_service_routine=None, debounce_delay=100,
                 active_HI=True, both_edge=False, repeat=False, repeat_delay=500):
        # print(f"@{utime.ticks_ms()}=Button__init__")
        self.name = name
        ...
        # print(f"@{self.last_change_time}>Button/last_change_time")
        ...
        if self.repeat == True:
            self.handler = self.irq_repeat_handler
        else:
    ...
    def irq_repeat_handler(self, pin):
        ...
        if last_event_delay >= self.debounce_delay:
            ...
            while self.signal.value()==self.is_pressed:
                self.interrupt_service_routine(self)
                utime.sleep_ms(int(self.repeat_delay))
            self.is_pressed = bool(self.signal.value())
            #print(self.is_pressed)
        self.pin.irq(handler=self.irq_repeat_handler)
